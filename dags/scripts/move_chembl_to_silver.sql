create or replace function bronze_move_molecules_properties()
returns varchar
language plpgsql
as $$
begin
    truncate table silver_molecule_properties;
    insert into silver_molecule_properties(
    ...
    from bronze_raw_mols_data
    where (molecule_structures::json->'canonical_smiles')::varchar <> '';
    return 'done';
end; 
$$;

create or replace function bronze_move_molecules_id()
returns varchar
language plpgsql
as $$
begin
    truncate table silver_chembl_id;
    insert into silver_chembl_id(chembl_id, smile)
    select distinct on
        (molecule_chembl_id) "molecule_chembl_id" as "chembl_id",
